...
Vref_index_start_int = fix(float1_0 * Vref_index(1));
Vref_index_end_int = fix(float1_0 * Vref_index(table_size));
Vref_index_step_int = fix(float1_0 * (Vref_index(2) - Vref_index(1)));
...
fprintf(txtfile,'%s\n','');
%write the macros
fprintf(txtfile,'%s','#define    OVM_GAIN_ARRAY_SIZE       ((uint16_t)');% macro 1
fprintf(txtfile,'%d',table_size);
fprintf(txtfile,'%s\n',')');

fprintf(txtfile,'%s','#define    OVM_ALPHA_ARRAY_SIZE      ((uint16_t)');% macro 2
fprintf(txtfile,'%d',alpha_table_size);
fprintf(txtfile,'%s\n',')');

fprintf(txtfile,'%s','#define    OVM_ALPHA_ARRAY_OFFSET    ((int16_t)');% macro 3
...
for i = 1:table_size
    data = fix(gain_table_data(i)*16384);
...
for i = 1:alpha_table_size
    data = fix(hold_table_data(i)*16384);
    fprintf(txtfile,'%d',data);
    fprintf(txtfile,'%s',',');
    if(mod(i,line_num) == 0)
        fprintf(txtfile,'%s\n', '\');
